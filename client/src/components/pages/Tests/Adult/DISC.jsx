...
const DISC = () => {
  ...
  const getPersonalityDescription = () => {
    ...
    return (
      <div className="disc-adult-personality-description">
        {/* <Sidebar /> */}
        <h3 className="disc-adult-h3">سبک اصلی: {descriptions[results.primary]}</h3>
        <h3 className="disc-adult-h3">سبک ثانویه: {descriptions[results.secondary]}</h3>
        <p className="disc-adult-p">
  ...
  const getDetailedAnalysis = () => {
    ...
    return (
      <div className="disc-adult-detailed-analysis">
        {/* <Sidebar /> */}
        <h3 className="disc-adult-h3">آنالیز تفصیلی سبک شخصیتی شما</h3>
        <div className="mb-4">
          <h4 className="disc-adult-h4">سبک اصلی: {results.primary}</h4>
          <p className="disc-adult-p">{detailedDescriptions[results.primary]}</p>
        </div>
        <div className="mb-4">
          <h4 className="disc-adult-h4">سبک ثانویه: {results.secondary}</h4>
          <p className="disc-adult-p">{detailedDescriptions[results.secondary]}</p>
        </div>
        <p className="disc-adult-p italic">این تحلیل به شما کمک می‌کند تا نقاط قوت خود را بهتر بشناسید و در مسیر رشد حرفه‌ای و شخصی قدم بردارید.</p>
      </div>
    );
  };
  ...
  if (showIntro) {
    return (
      <div className="disc-adult-wrapper">
        <div className="disc-adult-container">
          <div className="disc-adult-intro-section">
            <h1 className="disc-adult-h1">معرفی آزمون شخصیت‌شناسی DISC</h1>
            <p className="disc-adult-p text-gray-600 mb-4">
            ...
            <p className="disc-adult-p text-gray-600 mb-6">
            ...
            <div className="disc-adult-button-group">
              <button
                onClick={() => setShowIntro(false)}
                className="disc-adult-next-button"
              >
                شروع آزمون
              </button>
              <button
                onClick={() => navigate('/dashboard')}
                className="disc-adult-prev-button"
              >
                انصراف
              </button>
            </div>
  ...
  if (loading) {
    return (
      <div className="disc-adult-wrapper">
        <div className="disc-adult-loading">
  ...
  if (error) {
    return (
      <div className="disc-adult-wrapper">
        <div className="disc-adult-error">
          <p>{error}</p>
        </div>
      </div>
    );
    ...
    <div className="disc-adult-wrapper">
      <div className="disc-adult-container">
        ...
        {!isCompleted ? (
          <>
            <div className="mb-6">
              <h1 className="disc-adult-h1">آزمون شخصیت‌شناسی DISC</h1>
              <p className="disc-adult-p text-gray-600">لطفاً به هر سؤال با انتخاب یک گزینه بین 1 (کاملاً مخالفم) تا 5 (کاملاً موافقم) پاسخ دهید.</p>
              <div className="disc-adult-progress-bar">
                <div 
                  className="disc-adult-progress-bar-fill" 
                  style={{ width: `${progressPercentage()}%` }}
                ></div>
              </div>
              <div className="disc-adult-progress-text">
            ...
            <div className="mb-6">
              {getCurrentQuestions().map((question) => (
                <div key={question.id} className="disc-adult-question-card">
                  <div className="flex items-start mb-2">
                    <span className="disc-adult-question-number">{question.id}.</span>
                    <p className="disc-adult-p">{question.text}</p>
                  </div>
                  <div className="disc-adult-radio-group">
                    <span>کاملاً مخالفم</span>
                    <div className="disc-adult-radio-options">
                      {[1, 2, 3, 4, 5].map((value) => (
                        <label key={value} className="disc-adult-radio-label">
                          <input
                            type="radio"
                            ...
                            className="disc-adult-radio-input"
                          />
                          <span className={`disc-adult-radio-button ${answers[question.id] === value ? 'selected' : ''}`}>
                            {value}
                          </span>
                        </label>
                      ))}
            ...
            <div className="disc-adult-button-group">
              ...
              {currentPage * questionsPerPage >= questions.length ? (
                <button
                  ...
                  className="disc-adult-submit-button"
                >
                  مشاهده نتایج
                </button>
              ) : (
          ...
          <div className="disc-adult-results-container">
            <h2 className="disc-adult-h2">نتایج آزمون DISC</h2>

            {controlScore > 30 && (
              <div className="disc-adult-alert">
                <p className="disc-adult-p font-bold">توجه:</p>
                <p className="disc-adult-p">امتیاز شما در سؤالات کنترلی بالاست. ممکن است پاسخ‌های شما بیش از حد ایده‌آل بوده باشند.</p>
              </div>
            )}

            <div className="disc-adult-score-grid">
              {Object.keys(results.scores).map((category) => (
                <div key={category} className="disc-adult-score-card">
                  <h3 className="disc-adult-h3">{category}</h3>
                  <div className="disc-adult-score-bar">
                    <div
                      style={{ width: `${results.scores[category]}%` }}
                      className={`disc-adult-score-bar-fill ${category}`}
                    >
            ...
            {!showAnalysis ? (
              <div className="disc-adult-analysis-prompt">
                <h3 className="disc-adult-h3">آیا می‌خواهید آنالیز تفصیلی‌تری از نتایج خود ببینید؟</h3>
                <div className="disc-adult-button-group">
                  <button
                    onClick={() => {
              ...
              <>
                ...
                <div className="text-center mt-6">
                  <button
                    ...
                    className="disc-adult-restart-button"
                  >
                    شروع مجدد آزمون
                  </button>
                </div>
            ...
            <div className="disc-adult-history-section mt-8">
              <h3 className="disc-adult-h3">تاریخچه نتایج شما</h3>
              {history.length > 0 ? (
                <ul className="disc-adult-history-list">
                  {history.map((entry, index) => (
                    <li key={index} className="disc-adult-history-item">
                      <p className="disc-adult-p"><strong>زمان:</strong> {entry.timestamp}</p>
                      <p className="disc-adult-p"><strong>سبک اصلی:</strong> {entry.primary}</p>
                      <p className="disc-adult-p"><strong>سبک ثانویه:</strong> {entry.secondary}</p>
                      <p className="disc-adult-p"><strong>امتیازات:</strong> D: {entry.scores.D}%, I: {entry.scores.I}%, S: {entry.scores.S}%, C: {entry.scores.C}%</p>
                    </li>
                  ))}
                </ul>
              ) : (
                <p className="disc-adult-p">هنوز نتیجه‌ای ذخیره نشده است.</p>
              )}
            </div>
          </div>
        )}
